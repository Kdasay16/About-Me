...
if (responseOne) {
  alert('Awseome! Lets get started!');
} else {
...
function allQuestions () {
  ...
  if (quizResponseFive.toUpperCase() === 'Y' || quizResponseFive.toUpperCase() === 'YES') {
    alert('I cannot believe you think I would love veggies. Disappointed.');
  } else if(quizResponseFive.toUpperCase() === 'N' || quizResponseFive.toUpperCase() === 'NO') {
    alert('You are right! Veggies are pretty gross!');
  } else {
    alert('Please respond with a \'Y\' or \'N\'');
  }
};
...
var questionCount;
if (questionCount === 6) {
  prompt('Bonus question! You get six tries. What other states have I lived in?');
  allStates();
}
function allStates() {
